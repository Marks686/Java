...
public class CommunityCases extends InitAndEnd{
    ...
    * */
    ...
    @Order(2)
    @Test
    void ArticleList() throws InterruptedException {
     ...
     * url
     * 文章标题
     * 页面article_list_a_title是“文章详情页”
     */
    @Order(3)
    @ParameterizedTest
    @MethodSource("Generator")
    void ArticleDetail(String expected_url, String expected_title, String expected_blog_title) {
        // 找到第一篇文章对应的产看全文按钮
        webDriver.manage().timeouts().implicitlyWait(3, TimeUnit.SECONDS);
        ...
        webDriver.manage().timeouts().implicitlyWait(3, TimeUnit.SECONDS);
        // 获取文章标题
        String cur_blog_title = webDriver.findElement(By.cssSelector("#artical-items-body > div:nth-child(1) > div > div.col > div.text-truncate > a > strong")).getText();
    ...
    @Order(4)
    @Test
    void ArticleInfoChecked() {
        webDriver.get("http://1.14.165.126:58080/index.html");
        // 获取第一篇文章标题
        String first_blog_title = webDriver.findElement(By.cssSelector("#artical-items-body > div:nth-child(1) > div > div.col > div.text-truncate > a > strong")).getText();
        // 获取第一篇文章发布时间
        String first_blog_time = webDriver.findElement(By.xpath("//*[@id=\"artical-items-body\"]/div[1]/div/div[2]/div[2]/div/div[1]/ul/li[2]")).getText();
        // 校验文章标题是不是自动化测试
        Assertions.assertEquals("【JavaSE】抽象类和接口 （抽象类概念 抽象类语法 抽象类和普通类区别 抽象类的作用 接口使用 接口特性 实现多个接口 接口的继承）", first_blog_title);
        // 如果时间是2024-10-03年发布的，测试通过
        if(first_blog_title.contains("2024-10-03")) {
            System.out.println("测试通过");
        } else {
